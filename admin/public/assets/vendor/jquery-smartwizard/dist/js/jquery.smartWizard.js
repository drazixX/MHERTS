...
(function (factory) {
  ...
  var defaults = {
    ...
    toolbarSettings: {
      toolbarPosition: 'bottom',
      toolbarButtonPosition: 'right',
      showNextButton: false, // Disable the Next button
      showPreviousButton: true,
      toolbarExtraButtons: [ // Add an extra button for Submit
          $('<button>', {
              class: 'btn btn-info',
              text: 'Submit',
              click: function() {
...
value: function _setEvents() {
  ...
  this.main.find('.sw-btn-next').on("click", function (e) {
    ...
    if (_this.current_index === _this.steps.length - 1) {
      // Trigger submit button click event
      _this.main.find('.sw-btn-submit').click();
    } else {
      _this._showNext();
    }
  }); // Previous button event
  ...
  if (this.options.backButtonSupport) {
    $(window).on('hashchange', function (e) {
      ...
      //     this.main.find('.sw-btn-submit').removeClass("enabled");
      ...
      //       case 'last':
      //         this.main.find('.sw-btn-submit').addClass("enabled");
      //         break;
      ...
      // }
        ...
        value: function _setButtons(idx) {
          ...
          if (!this.options.cycleSteps) {
            this.main.find('.sw-btn-prev').removeClass("disabled");
            this.main.find('.sw-btn-next').addClass("disabled"); // Disable Next button by default
      
            if (this._getStepPosition(idx) === 'first') {
              this.main.find('.sw-btn-prev').addClass("disabled");
            } else if (this._getStepPosition(idx) === 'last') {
              this.main.find('.sw-btn-submit').addClass("enabled");
              this.main.find('.sw-btn-next').removeClass("disabled").hide(); // Hide Next button at the last step
            } else {
              this.main.find('.sw-btn-next').removeClass("disabled").show(); // Show Next button for intermediate steps
              this.main.find('.sw-btn-submit').removeClass("enabled"); // Disable Submit button for intermediate steps
            }
          }
        }
